...
def make_calendar_heatmap(dates, values, title="", colorscale=None, discrete_breaks=None,
    ...
    for i, yr in enumerate(years):
        ...
        # Black border around every cell that has a value, drawn as ONE
        # combined Scatter line-trace per year (rectangle outlines
        # separated by None to break the line between them) rather than
        # per-cell shapes or a second heatmap layer — trace draw order is
        # the most reliable way to guarantee something renders on top in
        # Plotly, and this stays in data coordinates so it scales correctly
        # with the chart regardless of container width.
        rows_idx, cols_idx = np.where(~np.isnan(z))
        bx, by = [], []
        for r, c in zip(rows_idx, cols_idx):
            bx += [c - 0.5, c + 0.5, c + 0.5, c - 0.5, c - 0.5, None]
            by += [r - 0.5, r - 0.5, r + 0.5, r + 0.5, r - 0.5, None]
        if bx:
