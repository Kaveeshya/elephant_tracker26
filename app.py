...
st.markdown("""
<style>
:root { --accent:#0d5c4a; --accent-light:#ecfeff; }

/* ── Global — projector-friendly dark text ── */
.stApp {
    background:#f0f4f3;
...
/* Force all text elements to be very dark */
p, span, div, label, li, td, th, caption, figcaption {
...
/* ── Headings ── */
h1, h2, h3, h4 { color: #0a0f1e !important; font-weight: 800; }

/* ── Section & sub titles ── */
.section-title {
    font-size: 32px; font-weight: 900; color: #0a0f1e !important;
    text-align: center; margin: 10px 0 16px; letter-spacing: -0.5px;
}
.sub-title {
    font-size: 24px; font-weight: 800; color: #0f172a !important;
    text-align: center; margin: 18px 0 10px;
}
.section-description {
    font-size: 16px; color: #1e293b !important; text-align: center;
    line-height: 1.8; max-width: 900px; margin: 0 auto 20px auto;
}

/* ── Cards ── */
.card {
    background: white; border-radius: 14px; padding: 20px 22px;
    margin-bottom: 18px; box-shadow: 0 2px 12px rgba(0,0,0,.10);
}
.card-title { color: #0d5c4a !important; font-size: 18px; font-weight: 800; margin-bottom: 8px; }
.card-note  { color: #1e293b !important; font-size: 13px; margin-bottom: 10px; line-height: 1.7; }
...
.vbox .num { font-size: 28px; font-weight: 900; line-height: 1.1; }
.vbox .lab { font-size: 14px; font-weight: 600; opacity: .95; margin-top: 3px; }

/* ── Metrics ── */
[data-testid="stMetricValue"]  { color: #0d5c4a !important; font-size: 26px !important; font-weight: 800 !important; }
[data-testid="stMetricLabel"]  { color: #0f172a !important; font-size: 14px !important; font-weight: 700 !important; }
...
/* ── Captions ── */
.stCaption, small { color: #334155 !important; font-size: 13px !important; }

/* ── Info/warning boxes ── */
.stAlert { color: #0f172a !important; font-weight: 600; }
...
def render_heat_tab():
    ...
    names_seq = d.groupby("name").size().sort_values(ascending=False).index.tolist()
...
def render_live_playback(df, live_elephant):
    ...
    st.components.v1.html(_build_live_map_component(df, live_elephant, clr), height=860, scrolling=False)
    card_close()
