...
def load_elephants(path="kaudulla_elephants_clean.csv"):
    df = pd.read_csv(path)
    # Preserve the literal wall-clock timestamp exactly as written in the
    # CSV (strip any trailing offset/'Z' without shifting the clock) —
    # this is what R's app effectively joins on, since it parses the GPS
    # date/time text columns and labels them "UTC" without ever actually
    # converting. Used as the join key in build_tk_dataset(); kept
    # separate from `datetime`/`datetime_sl` below, which DO apply a real
    # UTC conversion and are correct for display and everywhere else.
    raw_dt_str = df["datetime"].astype(str).str.replace(
        r"(Z|[+-]\d{2}:?\d{2})$", "", regex=True
    )
    df["datetime_naive"] = pd.to_datetime(raw_dt_str, format="ISO8601")
...
@st.cache_data(show_spinner=False)
def compute_hull_cached(lon_tuple, lat_tuple, ratio=0.3):
    """Cached wrapper around compute_hull(), keyed on the actual point
    coordinates (as hashable tuples) rather than a DataFrame. Streamlit
    hashes the tuple args, so calling this again with the *same*
    elephant's points on a later rerun (e.g. because a different
    elephant's checkbox changed, or an unrelated widget on the page
    fired) is a cache hit and skips the concave_hull() + geodesic area
    recomputation entirely."""
    df = pd.DataFrame({"lon": lon_tuple, "lat": lat_tuple})
    return compute_hull(df, ratio=ratio)
...
def build_tk_dataset(elephants_df, hourly_df):
    ...
    base_dataset inner join.

    Uses `datetime_naive` (the literal wall-clock timestamp as written in
    the CSV, with no timezone shift applied) as the join key — NOT
    `datetime` (which is a real UTC conversion). R's app parses its GPS
    date/time text columns and labels them "UTC" WITHOUT ever actually
    converting the clock, and the climate CSV is parsed the same naive
    way, so the two sides line up by construction. If we instead used the
    UTC-shifted `datetime` here, rows land in different hour buckets than
    R and the inner join below silently drops most of them."""
    d = elephants_df.copy()
    d["dt_round"] = d["datetime_naive"].dt.floor("h")
    d["date"] = d["datetime_naive"].dt.date
